...
class Links {
	...
	function update() {
		...
		if($this->id!="") {
			$sql = "UPDATE otherlinks SET message = \"". $this->message ."\" WHERE id=".$this->id."";
			$result = $db->query($sql);
	...
	function validId() {
		...
		$numrows = $db->getOne("SELECT count(*) FROM otherlinks WHERE id=".$this->id);
	...
	function init() {
		...
		if($this->id==null || $this->validId()==false)
			return false;
		$sql = "SELECT id, message FROM otherlinks WHERE id=".$this->id;
		$row = $db->getRow($sql);
		if (DB::isError($row)) {
			die ($row->getMessage());
		}
