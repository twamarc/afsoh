...
class Program {
	...
	function update() {
		...
		if($this->id!="") {
			$sql = 'UPDATE program SET message = \''. $this->message .'\' WHERE id='.$this->id.'';
			$result = $db->query($sql);
	...
	function validId() {
		...
		$numrows = $db->getOne("SELECT count(*) FROM program WHERE id=".$this->id);
	...
	function init() {
		...
		if($this->id==null || $this->validId()==false)
			return false;
		$sql = "SELECT id, message FROM program WHERE id=".$this->id;
		$row = $db->getRow($sql);
		if (DB::isError($row)) {
			die ($row->getMessage());
		}
